...
uses
  ...
  StdCtrls, TntStdCtrls, ExtCtrls, TntExtCtrls, JclDebug;
...
type
  ...
  TExceptionDialog = class(TTntForm)
    DetailsMemo: TTntMemo;
    Bevel1: TTntBevel;
    TextLabel: TTntMemo;
    Label1: TTntLabel;
    Panel1: TTntPanel;
...
// [...]
//   end else
//    SysUtils, TntSysUtils.ShowException(ExceptObject, ExceptAddr);
// end;
...
begin
  Result := PathExtractFileDirFixed(ParamStr(0)) + PathExtractFileNameNoExt(ParamStr(0)) + '_Err.log'; // do not localize
end;
...
var
  i,j: integer;
begin
  i := Canvas.TextWidth(TextLabel.Text) + TextLabel.Left + 32;
  j := Round(Screen.Width * 541 / 1000); // maximum widht
  if i < Constraints.MinWidth then i := Constraints.MinWidth
    else if i > j then i := j;
  Left := (Screen.Width - i) div 2;
  Width := i;
  case TextLabel.Lines.Count of
    1: begin
         TextLabel.Height := 44;
         TextLabel.Top := 16;
       end;
...
const
  MMXText: array[Boolean] of PChar = ('', 'MMX'); // do not localize
  FDIVText: array[Boolean] of PChar = (' [FDIV Bug]', ''); // do not localize
var
  SL: TStringList;
  I: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      for I := 0 to SL.Count - 1 do
      begin
        if I > 2 then break; // hack! only show modules under 'fma' directory (FMA and 2 DLLs)
        ModuleName := SL[I];
...
begin
  ...
  Caption := WideFormat(RsAppError, [TntApplication.Title]);
end;
...
begin
  DrawIcon(Canvas.Handle, TextLabel.Left - GetSystemMetrics(SM_CXICON) - 15,
    8{TextLabel.Top}, LoadIcon(0, IDI_ERROR));
end;
...
begin
  ...
  begin
    ...
    DetailsCaption := '<< ' + RsDetailsButton;
    ClientHeight := FFullHeight;
    if ClientWidth < 600 then begin
      Left := Left - (600 - ClientWidth) div 2;
      ClientWidth := 600;
    end;
...
begin
  ...
  try
    ...
    begin
      ...
      DetailsMemo.Lines.Add(Format(RsExceptionMessage, [E.Message]));      
      if Thread = nil then
...
begin
  ...
  begin
    ...
    if TextLabel.Lines.Count * Canvas.TextHeight('Wg') > TextLabel.ClientHeight then // do not localize
