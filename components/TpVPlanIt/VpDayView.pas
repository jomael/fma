...
type
  ...
  published
    ...
    property PopupMenuAdd: Boolean read FPopupMenuAdd write FPopupMenuAdd default False;
    {events}
...
begin
  if PopupMenu.Items.Count > 0 then PopupMenu.Items.NewTopLine;
...
begin
  ...
  if not Assigned (PopupMenu) then PopupMenu := TPopupMenu.Create(Self);

  if PopupMenu.Items.Count = 0 then PopupMenuAdd := True;

  if PopupMenuAdd then
  begin
    if  not dvPopupInited then
    begin
    ...
    if not Assigned (FActiveEvent) then
      for i := 0 to PopupMenu.Items.Count - 1 do begin
        if (PopupMenu.Items[i].Tag = 1) or (ReadOnly) then
  ...
  case Key of
    ...
    VK_F10    :
      if (ssShift in Shift) and not (Assigned (PopupMenu)) then begin
        ...
        PopupMenu.Popup (PopupPoint.x + 10,
                         PopupPoint.y + 10);
      end;
    VK_APPS   :
  ...
  var
    ...
    begin                                                                
      ...
      if (dvBmpCustom.Width <> 0) and                                    
        ...
        Canvas.StretchDraw(Rect (IconRect.Left + 1,
                                 IconRect.Top + 1,                         
                                 IconRect.Left + CustomW + 1,              
                                 IconRect.Top + CustomH + 1), dvBmpCustom);
        DrawPos := CustomW + 1;                                          
      end;                                                               
      ...
      if (dvBmpAlarm.Width <> 0) and (dvBmpAlarm.Height <> 0) then begin
        Canvas.StretchDraw(Rect (IconRect.Left + DrawPos,
                               IconRect.Top + 1,
                               IconRect.Left + DrawPos + AlarmW + 1,
                               IconRect.Top + AlarmH + 1), dvBmpAlarm);
  ...
  try
    ...
    while EventList.Count > 0 do begin
      ...
      while (J < EventList.Count) and (J < MaxVisibleEvents) do begin
        ...
        if StartTime < trunc(RenderDate) then StartTime := trunc(RenderDate);
        if StartTime - Trunc(StartTime) >= ThisTime then begin
          if EndTime > (trunc(RenderDate) + 1) then EndTime := trunc(RenderDate) + 1;
          ThisTime := EndTime - Trunc(EndTime);
...
/////// Debug Code /////////
  ...
  try
    ...
    for I := 0 to pred(MaxVisibleEvents) do begin
      ...
      EventSLine := GetStartLine(StartTime, Granularity);
      { remove the date portion from the start and end times }
      EventSTime := StartTime - trunc(StartTime);
      EventETime := EndTime - trunc(EndTime);
      { Handle End Times of Midnight }
