...
type
  ...
  protected{private}
    ...
    function GetCount: Integer;
    procedure Sort(EventList: TList); overload;
  public
...
procedure TVpSchedule.Sort(EventList: TList);
var
  i, j       : integer;
  IndexOfMin : integer;
  Temp       : pointer;
...
begin
  ...
  for i := 0 to pred(EventList.Count) do begin
    IndexOfMin := i;
    for j := i to EventList.Count - 1 do begin

      { compare start times of item[j] and item[i] }
      CompResult := Compare(TVpEvent(EventList.List^[j]).StartTime,
        TVpEvent(EventList.List^[IndexOfMin]).StartTime);
      ...
      { if the start times match then sort by end time }
        ...
        if (Compare(TVpEvent(EventList.List^[j]).EndTime,
          TVpEvent(EventList.List^[IndexOfMin]).EndTime) < 0)
        then
          IndexOfMin := j;
      end;
    end;

    Temp := EventList.List^[i];
    EventList.List^[i] := EventList.List^[IndexOfMin];
    EventList.List^[IndexOfMin] := Temp;
  end;

  { Fix object embedded ItemIndexes }
  for i := 0 to pred(EventList.Count) do begin
    TVpEvent(EventList.List^[i]).FItemIndex := i;
  end;
end;
{=====}
...
begin
  ...
  for I := 0 to pred(EventCount) do begin
    ...
    { inc result                                                      }
    else if (trunc(Value) >= trunc(Evnt.StartTime)) and (trunc(Value) < Evnt.EndTime) then
      Inc(Result);
  end;
end;
{=====}
...
begin
  if EventCountByDay(Date) = 0 then
    ...
    for I := 0 to pred(EventCount) do begin
      ...
      { the list.                                                          }
      else if (trunc(Date) >= trunc(Event.StartTime)) and (trunc(Date) < Event.EndTime) then
        EventList.Add(Event);
    end;
  end;
end;
