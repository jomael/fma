...
uses
 Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, janXPathTokenizer, Math, janstrings;
...
//  procedure parseNode(rootNode, parentNode: TjanXMLNode2);
  procedure ParseNode(Node: TJanXMLNode2);
...
begin
 ...
 // (Merijn) TODO:
 // - needs looping (check for more then 1 comment or version line...
 // - needs check for emtpy file
 ...
 repeat
  ...
  begin
   ...
   // skip any <!-- stuff (= comments)   (added by Merijn, 14 June 2004)
   if copy(Fxml, p, 4) = '<!--' then
   begin
    p2 := posstr('-->', Fxml, p);
    if p2 = 0 then
    begin
     result := 'Missing --> in xml comment.';
     exit;
    end
    else
    begin
   ...
   // skip any <!DOCTYPE stuff (added by Merijn, 20 August 2004)
   if copy(Fxml, p, 9) = '<!DOCTYPE' then
   begin
    p2 := posstr('>', Fxml, p);
    if p2 = 0 then
    begin
...
begin
 ...
 begin
  ...
  if p2 = 0 then
   raise exception.Create('Expected closing ' + delim + ' when parsing attributes');
...
procedure TJanXMLParser2.ParseNode(Node: TJanXMLNode2);

 // Merijn 8 July 2004: complete rewrote this procedure.
 // Because of a kind of strange recursive implementation in the original code
 // (always do a recursive call at the end of the procedure, not just to parse
 // a child and all it's nodes), a stack overflow would come if there was a xml
 // file with a certain amount of nodes.
 // Also added TTagType to make code more easy to read.

type TTagType = (ttTag, ttEndTag, ttShortcut);

var StartNode: TJanXMLNode2;
    Tag, TagName, Attributes: string;
    NewNode: TJanXMLNode2;
    TagType: TTagType;

 function GetTagType(Tag: string): TTagType;
 begin
  if Tag[1] = '/' then result := ttEndTag else
  if Tag[Length(Tag)] = '/' then result := ttShortcut else
  result := ttTag;
 end;

 function StripAttributes(var TagName: string): string;
 var AttributesStart: integer;
 begin
  result := '';
  AttributesStart := Pos(' ', TagName);

  if AttributesStart > 0 then
  begin // have attributes
   result := Trim(Copy(TagName, AttributesStart + 1, maxint));
   TagName := Copy(TagName, 1, AttributesStart - 1);
  end
 end;
 ...
 begin
  ...
  if fScan = 0 then raise Exception.Create('< not found');

  EndPos := PosStr('>', fXML, fScan);
  if EndPos = 0 then raise Exception.Create('> not found');

  result := Copy(fXML, fScan + 1, EndPos - fScan - 1);
  inc(fScan, EndPos - fScan + 1);
 end;
...
begin
 ...
 repeat
  ...
  case TagType of
   ttTag,       // <MyTag Attribute1="blah">
   ttShortCut:  // <MyTag Attribute1="blah" />
   begin
    if TagType = ttShortCut then
     Tag := Trim(Copy(Tag, 1, Length(Tag) - 1));; // trim last '/'

    TagName := Tag;
    Attributes := StripAttributes(TagName);

    if Node = nil then // we started with the root node, so use that
     NewNode := FparseRoot
    else
     NewNode := TjanXMLNode2.Create();  // else make a new one
 ...
 until Node = StartNode;   // this means we're done parsing up to _and including_ the start node
end;
...
procedure TjanXMLParser2.setNodeXML(pNode: TjanXMLNode2; const Value: string);
begin
 Fxml := value;
...
begin
 ...
 begin
  ...
  case tok.tokenkind of
   tkOperand:
    if not InFixToPostFix(i) then
     exit;
   tkOpen:
    if not InFixToStack(i) then
     exit;
   tkClose:
    if not CloseStackToPostFix then
     exit;
   tkOperator:
    begin
...
begin
 ...
 begin
  ...
  if (copy(s1, 1, 1) = '%') and (copy(s1, length(s1), 1) = '%') then
  begin
